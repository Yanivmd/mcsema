...
rem    %BIN_DESCEND_PATH%\bin_descend_wrapper.py  -d -i=%filename%.%ext%  -entry-symbol=_main 
	rem %BIN_DESCEND_PATH%\bin_descend_wrapper.py -b -d -func-map="%STD_DEFS%" -entry-symbol=_doCommand1 -i=%filename%.obj
...
rem ) else (
    ...
    %BIN_DESCEND_PATH%\bin_descend.exe -d -l -entry-symbol=_main -i=%filename%.obj
	rem exit
rem )

%CFG_TO_BC_PATH%\cfg_to_bc.exe -i %filename%.cfg -driver=test_entry,_main,0,return,C -o %filename%.bc  -ignore-unsupported     
